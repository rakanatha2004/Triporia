<section>
    <div class="bg-gray-100 min-h-screen p-6">
      <div class="max-w-full lg:max-w-5xl mx-auto">
        <h2 class="text-2xl lg:text-3xl font-semibold mb-8 text-center">
          Other Tourist Recommendation
        </h2>
  
        <!-- Grid: 1 kolom mobile, 2 kolom sm+ -->
        <div class="grid grid-cols-1 sm:grid-cols-1 gap-8">
          <!-- Card 1 -->
          <div class="bg-white rounded-3xl shadow-lg overflow-hidden p-6 flex items-start space-x-6">
            <img
              src="assets/images/calf.svg"
              alt="Kopi Calf"
              class="w-32 h-32 object-cover rounded-xl flex-shrink-0"
            />
            <div>
              <h3 class="text-lg lg:text-xl font-semibold mb-2">Kopi Calf Signature</h3>
              <div class="flex items-center text-base text-red-600">
                <img
                  src="{{asset('assets/images/🦆 icon _location tick_.svg')}}"
                  alt="Location"
                  class="w-5 h-5 mr-2"
                />
                <span>Jl. Prof. Dr. Suharso</span>
              </div>
            </div>
          </div>
  
          <!-- Card 2 -->
          <div class="bg-white rounded-3xl shadow-lg overflow-hidden p-6 flex items-start space-x-6">
            <img
              src="{{asset('assets/images/alas.svg')}}"
              alt="Alas House"
              class="w-32 h-32 object-cover rounded-xl flex-shrink-0"
            />
            <div>
              <h3 class="text-lg lg:text-xl font-semibold mb-2">Alas House</h3>
              <div class="flex items-center text-base text-red-600">
                <img
                  src="{{asset('assets/images//🦆 icon _location tick_.svg')}}"
                  alt="Location"
                  class="w-5 h-5 mr-2"
                />
                <span>Jl. Gelora Indah I No.36</span>
              </div>
            </div>
          </div>
  
          <!-- Card 3 -->
          <div class="bg-white rounded-3xl shadow-lg overflow-hidden p-6 flex items-start space-x-6">
            <img
              src="{{asset('assets/images/etall.svg')}}"
              alt="et al Coffee Purwokerto"
              class="w-32 h-32 object-cover rounded-xl flex-shrink-0"
            />
            <div>
              <h3 class="text-lg lg:text-xl font-semibold mb-2">et al Coffee Purwokerto</h3>
              <div class="flex items-center text-base text-red-600">
                <img
                  src="{{asset('assets/images//🦆 icon _location tick_.svg')}}"
                  alt="Location"
                  class="w-5 h-5 mr-2"
                />
                <span>Jl. DI Panjaitan No.23</span>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
  